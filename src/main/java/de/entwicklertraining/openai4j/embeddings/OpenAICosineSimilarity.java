 ...
 */
public final class OpenAICosineSimilarity {
     ...
     */
    public OpenAICosineSimilarity(OpenAIClient client) {
        this(client, EmbeddingModel.TEXT_EMBEDDING_3_SMALL);
    }
    /**
     * Creates an instance specifying the embedding model to use.
     */
    public OpenAICosineSimilarity(OpenAIClient client, EmbeddingModel model) {
        this.client = client;
        this.model = model;
    }
    ...
    private double[] embeddingFor(String text) {
        return cache.computeIfAbsent(text, t -> {
            OpenAIEmbeddingsResponse resp = client.embeddings()
                    .model(model)
                    .addInput(t)
