 ...
 * Typical usage:
 * <ol>
 *   <li>Create an instance with a {@link OpenAIClient}.</li>
 *   <li>Invoke {@link #handleRequest(OpenAIChatCompletionRequest, boolean)} with an
 *       initial request.</li>
 *   <li>Process the returned {@link OpenAIChatCompletionResponse} which either
 *       contains the final assistant message or indicates a refusal.</li>
 * </ol>
 */
public final class OpenAIChatCompletionCallHandler {
     ...
     */
    public OpenAIChatCompletionCallHandler(OpenAIClient client) {
        this.client = client;
    }
     ...
     */
    public OpenAIChatCompletionResponse handleRequest(OpenAIChatCompletionRequest initialRequest,
        ...
        OpenAIChatCompletionRequest currentRequest = initialRequest;
        int turnCount = 0;
        ...
        while (true) {
            ...
            for (int i = 0; i < toolCallsArray.length(); i++) {
                ...
                // Tool aufrufen
                OpenAIToolDefinition toolDef = toolMap.get(toolName);
                OpenAIToolResult toolResult = toolDef.callback().handle(new OpenAIToolCallContext(args));
     ...
     */
    private OpenAIChatCompletionRequest buildNextRequest(OpenAIChatCompletionRequest initialReq,
                                                      List<org.json.JSONObject> messages) {
        var builder = OpenAIChatCompletionRequest.builder(client)
